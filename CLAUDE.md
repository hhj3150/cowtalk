...
이 플랫폼은 1개 농장 → 전국 → 국가로 확장됩니다.

## 현재 규모 (실측 스냅샷)

⚠️ 규모 수치의 유일한 진실은 DB다 — 화면·문서에 규모를 하드코딩하지 말고
GET /api/public/stats (실시간 DB 집계, 5분 갱신)를 사용한다.
아래는 참고용 스냅샷 (2026-07-19 랜딩 페이지 실측):
- 농장 201개 / 소 11,170두 / 센서 11,127두 / 누적 이벤트 757,062건 / AI 엔진 6개
- 과거 문서·주석의 "146개 농장 / 7,000두"는 옛 스냅샷 — 현재 수치로 읽지 말 것

## 파일럿 (2026-07 시작)

- 소유자 프로필: **스마트팜 전문가 · 축산환경 전문가**, 송영신목장과 **우유공장(자체 가공·판매)** 직접 운영
  → 자체가공 단가 구조가 실사업 구조와 일치. 축산환경(분뇨·탄소·악취) 도메인은
    소유자 전문 분야 — 향후 환경 모듈·경기도 제안서의 차별화 축으로 활용 가능
- 환경 축 공식 명칭 (⚠️ 용어 규칙): "피트모스" 직접 표현 대신
  **"휴머스(HUMUS)를 활용한 HBP 시스템(Humus Bedded Pack barn)"**을 사용한다.
  프레이밍: HBP 시스템 기반 **축산환경관리 및 경축순환모델 확립** — 적용 축종: 낙농·비육우·육계·양돈.
...
- **술탄목장: 2026-08 센서 45두 추가 삽입** — 정기 보고서의 '센서 장착' 행에 기간 내 증가분으로 표기된다
  (주간/월간/분기/성과/연간 5종을 hhj3150@hanmail.net 으로 **1년간** 자동 발송 — 아래 '정기 보고서 자동 발송' 참조)
...
- 착유 설비: **Lely A3 로봇착유기 1대 + T4C v4.20** (온프레미스, 공개 API 없음 — A3는 Horizon 대상 아님)
  - 유량 유입 경로: ① T4C 보고서 CSV 내보내기 → /milk-entry 업로드 (열 자동 인식/수동 매핑 지원)
    ② (예정) 목장 PC 동기화 에이전트: T4C 예약 내보내기 폴더 감시 → CowTalk API 전송
...
- AI: Anthropic Claude API (핵심 해석 엔진) + v4 룰 엔진 (fallback/보조)
  ...
  - ⚠️ 모델별 파라미터 분기는 `ai-brain/claude-model-params.ts` 한 곳에만 둔다.
    ...
    판정 규칙은 "구형만 명시, 모르는 모델은 최신 취급" (오판 시 400 대신 품질 손해로 떨어지게)
  - 롤백: 환경변수 ANTHROPIC_MODEL / ANTHROPIC_MODEL_DEEP 로 즉시 이전 세대 복귀 가능
- 테스트: Vitest
...
3층 — Role-Based Serving:
  4개 역할별 맞춤 대시보드 + 액션플랜
  - farmer: smaXtec 기본 기능(개체 모니터링, 할 일, 건강 알림) + AI 강화 + 번식 관리
  - veterinarian: 담당 목장 그룹 관리, 역학 관리(공수의사), 질병 패턴 분석, Command Center
  - quarantine_officer: 전국 농장 통합 방역, 역학 조사, 확산 시뮬레이션, 전국 현황
...
- 전체 농장 실좌표 CircleMarker (두수 비례 크기, 위험등급 색상)
...
| 주기 | 대상 기간 | 발송 시점 | 1년 횟수 | periodKey |
|---|---|---|---|---|
| weekly | 직전 월~일 (KST) | 월요일 07시 (KST, 기본값) | 52 | `weekly:2026-W34` |
| monthly | 직전 달 | 매월 1일 | 12 | `monthly:2026-07` |
| quarterly | 직전 분기 | 1·4·7·10월 1일 | 4 | `quarterly:2026-Q2` |
| performance | 직전 달 + **파일럿 누적** | 매월 1일 | 12 | `performance:2026-07` |
| annual | 직전 캘린더 연도 | 매년 1월 1일 | 1 | `annual:2026` |
...
- **발송 예정표**: `GET /reports/schedules/calendar?months=12` — 앞으로 1년 동안 언제 무슨 보고서가 오는지.
  실제 발송(`isDue`)과 **같은 규칙**을 미래로 투영한다 (예정표와 실제가 다른 규칙을 쓰면 예정표는 거짓말이 된다)
...
- UI: `/settings/reports` — 주기별 구독·구독 기간(1년/2년/무기한)·즉시 발송·**향후 12개월 예정표**·발송 이력(테스트모드까지 그대로 노출)
- 시드: 술탄목장 5종(주간/월간/분기/성과/연간) 구독 + 수신자 hhj3150@hanmail.net + 구독 기간 1년 (마이그레이션 0038·0039, 멱등)
