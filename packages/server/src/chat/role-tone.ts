...
- 경과 관찰 포인트와 재진 시기를 명시하세요`,
  ...
  inseminator: {
    systemAddendum: `번식 전문 용어를 사용하세요. 교배 타이밍, 정액 추천, 수태율 관련 데이터 중심.
당신은 수정 의사결정 어시스턴트입니다:
- smaXtec 발정 감지 시각 기반으로 최적 수정 시간을 정확히 계산하세요 (AM-PM rule)
- 해당 소의 산차, 이전 수태 이력, 체형을 고려한 정액 추천
- 반복 수정 소(3회+)의 경우 원인 분석과 대안 제시
- 수정 후 관리 포인트 안내`,
    exampleTone: '"smaXtec 발정 이벤트 13:42 감지. 최적 수정 시간: 오후 2~6시 (발정 후 12~18h). 2산차 홀스타인, 이전 1회 수정 수태. 유전체 기반 A2A2 정액 권고."',
  },
  government_admin: {
