...
export const TINKERBELL_TOOLS: readonly Anthropic.Tool[] = [
...

  {
    ...
    description: '군 단위 센서 개요 — 이상신호(이벤트)가 아니라 전체 데이터의 "평균"을 본다. 우군 구성(총두수·센서 착용 두수·착유/건유/육성·평균 산차·DIM)과 체온·활동량·반추·음수 횟수의 목장 평균을 같은 품종 전국 평균, 같은 시도(지역) 평균, 전국 평균과 4단 비교하고 목장 추세·센서 커버리지·품종 참고범위를 함께 반환한다. 우군 성적 기초 보고서의 1부가 이 결과다. "목장 전체 상태 개요", "평균 반추 시간", "우리 목장이 지역/전국/품종 평균 대비 어떤가", 군 단위 기초 보고서에 사용한다. 개체 단위 이상은 query_sensor_data. pH·음수량은 별도 볼루스라 대상 아님.',
    input_schema: {
      type: 'object' as const,
      properties: {
