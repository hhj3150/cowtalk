...
사람과 대화하듯 자연스러운 문장으로 답하세요. 짧은 질문에는 짧게, 설명이 필요하면
그때만 길게. 위 컨텍스트에 나온 수치·알람·통계를 근거로 쓰고, 컨텍스트에 없는 내용은
당신이 이미 아는 축산·수의학 지식으로 보충하세요. 개체를 말할 때는 농장명과 귀표번호를
같이 쓰고, 중요한 수치만 굵게 표시합니다. 이모지나 기호 아이콘, 표, ASCII 차트,
"키: 값" 나열은 쓰지 마세요. 사용자가 쓴 언어로 답하고 언어를 섞지 마세요.`;
  }
...
사람과 대화하듯 자연스러운 문장으로 답하세요. 짧은 질문에는 짧게, 설명이 필요하면
그때만 길게. 위 컨텍스트의 수치·알람을 근거로 쓰고, 없는 내용은 당신의 축산·수의학
지식으로 보충하세요. 이모지·기호 아이콘·표·ASCII 차트·"키: 값" 나열은 쓰지 마세요.
사용자가 쓴 언어로 답하세요. answer 필드는 단답형 질문이면 한 줄, 설명 요청이면
자연스러운 단락 형태로 작성합니다.
...
function buildQuarantineContextPrompt(data: QuarantineContextData): string {
  const RISK_LABEL: Readonly<Record<string, string>> = {
    green: '정상', yellow: '주의', orange: '경계', red: '위험',
  };
  const riskLabel = RISK_LABEL[data.kpi.riskLevel] ?? '미분류';

  const lines: string[] = [
    `## 맥락: 방역 모니터링 (전국 ${String(data.nationalSummary.totalFarms)}개 농장, ${String(data.nationalSummary.totalAnimals)}두)`,
    '',
    `### 방역 KPI (실시간)`,
    `- 위험 등급: ${riskLabel} **${data.kpi.riskLevel.toUpperCase()}**`,
  ...
  if (riskyProvinces.length > 0) {
    ...
    for (const p of riskyProvinces) {
      const pLabel = RISK_LABEL[p.riskLevel] ?? '미분류';
      lines.push(`- (${pLabel}) **${p.province}**: ${String(p.farmCount)}농장, 발열 ${String(p.feverAnimals)}두 (${String((p.feverRate * 100).toFixed(1))}%)`);
    }
  }
  ...
  if (data.targetProvince && data.provinceDetail && Array.isArray(data.provinceDetail) && data.provinceDetail.length > 0) {
    ...
    for (const d of data.provinceDetail as Array<{ district: string; farmCount: number; feverAnimals: number; feverRate: number; riskLevel: string }>) {
      const dLabel = RISK_LABEL[d.riskLevel] ?? '미분류';
      lines.push(`- (${dLabel}) ${d.district}: ${String(d.farmCount)}농장, 발열 ${String(d.feverAnimals)}두 (${String((d.feverRate * 100).toFixed(1))}%)`);
    }
  }
...
function buildAnimalContext(profile: AnimalProfile): string {
  ...
  if (profile.sensorHistory7d.length > 0) {
    ...
    if (temps7.length >= 2) {
      ...
      if (Math.max(...temps7) - Math.min(...temps7) > 1.0) {
        lines.push(`  →  7일 체온 변동폭 ${(Math.max(...temps7) - Math.min(...temps7)).toFixed(1)}°C — 간헐적 감염·발정·내분비 이상 감별`);
      }
    }
    if (rums7.length >= 2) {
      ...
      if (avg < 250) {
        lines.push(`  →  7일 평균 반추 ${avg.toFixed(0)}분 — 만성 반추위 산증·케토시스·제4위 변위 가능성 높음`);
      }
    }
    if (acts7.length >= 2) {
  ...
  if ((profile.sensorHistory30d ?? []).length >= 7) {
    ...
    if (temps30.length >= 7) {
      ...
      if (feverDays >= 5) {
        lines.push(`  →  30일간 발열 ${String(feverDays)}회 — 만성 감염(자궁내막염·유방염·폐농양) 또는 면역 저하 검토`);
      }
    ...
    if (rums30.length >= 7) {
      ...
      if (lowRumDays >= 7) {
        lines.push(`  →  30일 중 ${String(lowRumDays)}일 저반추 — 만성 SARA·케토시스·영양 불균형. TMR 배합비 즉시 분석 권고`);
      }
  ...
  // ── 기간별 비교 분석 (체온 기준, 프로필 센서 히스토리에서 직접 계산) ──
  {
    ...
    if (hist30.length >= 7) {
      ...
      if (latestTemp != null) {
        ...
        lines.push(`- 현재 상태: ${assessment.interpretation} (${assessment.deviationSigma.toFixed(1)}σ, ${assessment.withinNormal ? '범위 내' : ' 범위 이탈'})`);
      }
  ...
  if (profile.breedingHistory.length > 0) {
    ...
    if (failCount >= 2) {
      lines.push(`  →  수정 실패 ${String(failCount)}회 — 반복 수정 소(Repeat Breeder) 가능성: 자궁염·황체부전·위내 산증·영양 불균형 감별 필요`);
    }
  }
  ...
  if (profile.healthHistory.length > 0) {
    ...
    if (mastitisCount >= 2) {
      lines.push(`  →  유방염 반복 (${String(mastitisCount)}회) — 도태 검토, 미생물 감수성 검사, 건유 요법 재설계 필요`);
    }
  }
...
**[알람별 즉시 대응 프로토콜]**
- 체온 알람(>39.8°C): 직장체온 재확인 → 유방 4분방 검사(CMT) → 자궁 초음파 → 혈액검사(WBC, 피브리노겐)
...
- 건강 경고: 위 체온·반추·활동 종합 판단 → 감별진단 순서로 진행
