...
async function queryTodoList(
  ...
  const typeToTodo: Record<string, { label: string; category: string; icon: string; severity: TodoItem['severity']; priority: number }> = {
    ...
    calving_detection: { label: '🔴 분만 임박 — 분만실 이동', category: 'fertility', icon: 'baby', severity: 'critical', priority: 1 },
    calving_confirmation: { label: '🔴 분만 완료 — 초유 급여 확인', category: 'fertility', icon: 'baby', severity: 'critical', priority: 2 },
    temperature_high: { label: '🟠 발열 소 확인 — 체온 측정·격리', category: 'health', icon: 'thermometer', severity: 'high', priority: 3 },
    clinical_condition: { label: '🟠 질병 의심 — 수의사 진료 요청', category: 'health', icon: 'heart-pulse', severity: 'high', priority: 4 },
    health_general: { label: '🟠 건강 주의 — 경과 관찰', category: 'health', icon: 'heart-pulse', severity: 'high', priority: 5 },
    // 높음 — 금일 중 조치
    insemination: { label: '🟢 수정 완료 기록 확인', category: 'fertility', icon: 'venus', severity: 'medium', priority: 6 },
    rumination_decrease: { label: '🟡 반추 감소 — 사료·건강 점검', category: 'feeding', icon: 'utensils', severity: 'medium', priority: 7 },
    activity_decrease: { label: '🟡 활동 감소 — 파행·질병 확인', category: 'health', icon: 'activity', severity: 'medium', priority: 8 },
    temperature_low: { label: '🟡 저체온 — 신생우 보온 조치', category: 'health', icon: 'thermometer', severity: 'medium', priority: 9 },
    fertility_warning: { label: '🟡 번식 주의 — 발정 재관찰', category: 'fertility', icon: 'venus', severity: 'medium', priority: 10 },
    pregnancy_check: { label: '🟢 임신 검진 예정', category: 'fertility', icon: 'venus', severity: 'info', priority: 11 },
    no_insemination: { label: '🟡 발정 미수정 — 원인 파악', category: 'fertility', icon: 'venus', severity: 'medium', priority: 12 },
    dry_off: { label: '🟢 건유 전환 관리', category: 'fertility', icon: 'venus', severity: 'info', priority: 13 },
    activity_increase: { label: '🟢 활동 증가 — 발정 가능성 확인', category: 'fertility', icon: 'activity', severity: 'info', priority: 14 },
  };
