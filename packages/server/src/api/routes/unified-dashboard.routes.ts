...
import { resolveFarmProvince } from '../../services/epidemiology/province-mapper.js';
...
unifiedDashboardRouter.get('/farms', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const farmList = farmRows.map((f) => ({
      ...
      // 시도 부여: 유효 region > 주소 시·군명 > 좌표 (resolveFarmProvince 단일 규칙)
      province: resolveFarmProvince({ regionProvince: f.province, address: f.address, lat: f.lat, lng: f.lng }),
    }));
