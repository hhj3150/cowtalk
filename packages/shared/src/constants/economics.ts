...
export const ECONOMIC_PARAMETER_DEFS: readonly EconomicParameterDefinition[] = [
  ...
  {
    key: 'milk_fat_adjust_krw_per_01pct',
    labelKo: '유지방 가감 (0.1%p당)',
    unit: '원/L',
    defaultValue: 10,
    min: 0,
    max: 1000,
    basis: '원유 기본가 유지방(3.4%) 초과·미달 0.1%p당 가감액 근사. 유대명세서 기준으로 수정 권장.',
  },
  {
    key: 'scc_grade1_bonus_krw_per_l',
    labelKo: '체세포 1등급 가산',
    unit: '원/L',
    defaultValue: 50,
    min: 0,
    max: 1000,
    basis: '체세포 1등급(20만/mL 이하) 위생 가산액 근사. 유대명세서 기준으로 수정 권장.',
  },
  {
    key: 'milk_quota_price_krw_per_l',
    labelKo: '납유 단가 (쿼터 내)',
    unit: '원/L',
    defaultValue: 1084,
    min: 0,
    max: 10000,
    basis: '기준원유량(쿼터) 내 납유 단가 — 음용유 기본가 근사. 조합·유업체 계약 단가로 수정.',
  },
  {
    key: 'milk_surplus_price_krw_per_l',
    labelKo: '가공유 단가 (쿼터 초과)',
    unit: '원/L',
    defaultValue: 887,
    min: 0,
    max: 10000,
    basis: '쿼터 초과분 가공유용 원유 가격 근사 (용도별 차등가격제). 실제 정산 단가로 수정.',
  },
  {
    key: 'milk_self_price_krw_per_l',
    labelKo: '자체가공 환산 단가',
    unit: '원/L',
    defaultValue: 0,
    min: 0,
    max: 100000,
    basis: '직접 가공·판매 농가 전용 — 가공품 매출을 원유 L당으로 환산해 입력 (0 = 미사용).',
  },
  {
    key: 'daily_quota_l',
    labelKo: '1일 쿼터량 (기준원유량)',
    unit: 'L/일',
    defaultValue: 0,
    min: 0,
    max: 100000,
    basis: '조합·유업체 배정 1일 기준원유량. 0 = 미설정(전량 납유 단가 적용).',
  },
  {
